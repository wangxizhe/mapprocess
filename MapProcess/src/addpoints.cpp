...
void ugv_position::Startcallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& initial)
{
    ...
    map_final->points.push_back(initial_point);
}
void ugv_position::Goalcallback(const geometry_msgs::PoseStamped::ConstPtr& end)
{
    ...
    {
       if(map_final->points[i].x==initial_point.x&&map_final->points[i].y==initial_point.y)
       {
          continue;
       }
       else
...
void ugv_position::pub_cloud()
{
  while(ros::ok())
  {
    ...
    map_temp = map_final;
    map_temp->height = 1;
    map_temp->width = map_final->size() ;
    pcl::toROSMsg(*map_temp, ros_msgs);
